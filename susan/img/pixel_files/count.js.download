...
(function(){
	...
	function getWiredMinds() {
		var wiredminds = {
			...
			app_server : "wmapp.wiredminds.de",
                        cnt_server : "wm.wiredminds.de",
                        cnt_server_next : "t2.leadlab.click",
			app_server_proto : "https",
			cnt_server_proto : "https",
			numberOfCalls : 0,
			wm_track_url : "",
			wm_track_alt : "",
			wm_cvid : 0,
			...
			count_URL : function (getlist) {
                            this.sendRequest(this.cnt_server, getlist);
                            if (this.cnt_server_next && (this.cnt_server_next != "") && (this.cnt_server != this.cnt_server_next)){
                                this.sendRequest(this.cnt_server_next, getlist);
                            }
			},
                        
                        sendRequest : function(cnt_server, getlist){
                            var proto = wiredminds.getProtoCnt();
                            var WMCT_URL = proto + '//' + cnt_server + '/track/ctin.php?' + getlist;
			...
			internalCount : function (ext) {
				...
				if (wiredminds.getTrackParam('wm_ec_unit_price_in_currency') !== null)  
{wiredminds.add_to_list('wm_ec_unit_price_in_currency',wiredminds.getTrackParam('wm_ec_unit_price_in_currency'));}
				...
				if (wiredminds.getTrackParam('wm_ec_total_price_in_currency') !== null) 
{wiredminds.add_to_list('wm_ec_total_price_in_currency',wiredminds.getTrackParam('wm_ec_total_price_in_currency'));}
